...
public static class PurposeHeuristics
{
    ...
    private static (string Purpose, string Source) Derive(FileNode file, string content)
    {
        ...
        // Folder-name convention (e.g. Controllers/, Migrations/) — a reasonable role hint
        // when the file itself carries no comment, doc or name-convention signal.
        var folderRole = FolderConvention(file.RelPath);
        if (folderRole.Length > 0 && file.Types.Count > 0)
        {
            return ($"{Capitalize(folderRole)} ({file.Types[0].Name}).", "folder convention");
        }
        ...
        if (file.Types.Count > 0)
        {
            ...
            var methodCount = file.Types.Sum(t => t.Methods.Count);
            var api = methodCount > 0 ? $" (exposes {methodCount} method{(methodCount == 1 ? "" : "s")})" : "";
            return ($"{file.Language} source defining {string.Join(", ", names)}{more}{api}.", "declared types");
        }

        if (folderRole.Length > 0) { return ($"{Capitalize(folderRole)}.", "folder convention"); }
    ...
    private static readonly Dictionary<string, string> FolderConventions = new(StringComparer.OrdinalIgnoreCase)
    {
        ["Controllers"] = "HTTP endpoint controller",
        ["Services"] = "application/business service",
        ["Repositories"] = "data access repository",
        ["Models"] = "data model",
        ["Migrations"] = "database migration",
        ["Middleware"] = "request pipeline middleware",
        ["Handlers"] = "message/event handler",
    ...
    /// <summary>Role implied by any folder in the path matching a known convention name.</summary>
    private static string FolderConvention(string relPath)
    {
        foreach (var seg in relPath.Split('/'))
        {
            if (FolderConventions.TryGetValue(seg, out var role)) { return role; }
        }
        return "";
    }
