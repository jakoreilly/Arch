...
/// <see cref="HasInlineComments"/> reports when this happens so a caller can surface it.</summary>
public static class TSqlFormatter
{
    ...
    private const string PlaceholderPrefix = "ARCHFMT_BLOCK_";
    ...
    private sealed class BeginEndCommentSplicer : TSqlFragmentVisitor
    {
        ...
        private readonly Sql160ScriptGenerator _gen;
        private readonly Dictionary<string, string> _replacements;
        private int _nextMarker;

        public BeginEndCommentSplicer(IList<TSqlParserToken> tokens, Sql160ScriptGenerator gen, Dictionary<string, string> replacements)
        {
            _tokens = tokens;
            _gen = gen;
        ...
        public override void ExplicitVisit(BeginEndBlockStatement node)
        {
            ...
            foreach (TSqlStatement stmt in statements)
            {
                ...
                _gen.GenerateScript(stmt, out var formatted);
                sb.Append(formatted.TrimEnd());
                sb.Append('\n');
                cursor = stmt.LastTokenIndex + 1;
            }
